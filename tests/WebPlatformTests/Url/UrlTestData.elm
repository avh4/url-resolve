module WebPlatformTests.Url.UrlTestData exposing (data, json)

import Json.Decode as Decode exposing (Decoder)
import Url exposing (Url)


{-| From <https://github.com/web-platform-tests/wpt/blob/64ee2ba56dc79fd62f1b3722ff1a7a98c290b9f1/url/resources/urltestdata.json>
-}
type alias TestCase =
    { base : Maybe (Result String Url)
    , input : String
    , result : Result () String
    }


decodeTestCase : Decoder TestCase
decodeTestCase =
    Decode.map3 TestCase
        (Decode.field "base" (Decode.nullable decodeUrl))
        (Decode.field "input" Decode.string)
        decodeResult


decodeUrl : Decoder (Result String Url)
decodeUrl =
    Decode.oneOf
        [ Decode.string
            |> Decode.andThen (Url.fromString >> Result.fromMaybe "Invalid URL" >> fromResult)
            |> Decode.map Ok
        , Decode.string
            |> Decode.map Err
        ]
...
decodeResult : Decoder (Result () String)
decodeResult =
    Decode.maybe (Decode.field "failure" Decode.bool)
        |> Decode.andThen
            (\b ->
                case b of
                    Just True ->
...
data =
    let
        decodeItem : Decoder (Maybe TestCase)
        decodeItem =
            Decode.oneOf
                [ Decode.map Just decodeTestCase
                , Decode.map (always Nothing) Decode.string
                ]
    in
    case Decode.decodeString (Decode.list decodeItem) json of
        Ok a ->
            List.filterMap identity a
...
json =
  ...
  {
    "input": "http://example.com/\u{FEFF}/foo",
    "base": "about:blank",
